...
此案例通过构建一个随机森林分类器来预测汽车的安全

## 1.介绍

随机森林是一种基于集合学习的监督机器学习算法.在这个案例中,构建了两个随机森林分类器来预测汽车的安全性,一个有10个决策树,另一个有100个决策树.随着模型中决策树的数量,准确率也会相对增加,利用随机森林模型筛选特征的过程,寻找重要特征,删去无用特征,利用筛选后的特征重建模型,准确率的也有相对应的影响,不一定是增加,删除多了可能会减少

## 2.简介

随机森林有两种变体,一种解决分类问题,另一种解决回归问题,它是较灵活的算法,在给定的数据集上创建决策树,从每棵树中获取预测,并通过投票选择最佳解,类似于偏科战士各司其职,将多个决策树结合起来,形成森林,因此得名随机森林(`Random Forest`),在随机森林分类器中,森林中树木数量越多,准确率越高.

## 3.算法直觉

随机森林可以分为两个阶段:

第一阶段:从`m`个特征中随机选择`k`个特征,并构建随机森林,以下步骤
...
1. 解决分类和回归问题
2. 使用了大量决策树来进行预测,所以模型是比较准确的
3. 随机森林取所有决策树预测的平均值,从而抵消偏差,因此,没有过拟合的问题
4. 处理缺失值
   - 使用中位数进行替换连续变量
   - 计算缺失值的邻近加权平均值
5. 随机森林用于特征选择,从训练集中选择最重要的特征
...
随机森林可用于特征选择的过程,算法可对于回归或者分类问题中变量的重要性进行排序,我们将通过随机森林算法和到数据中来衡量数据集中的变量重要性,在拟合过程中,记录每个数据点的袋外误差,并在森林中取平均值
...
3. 列名重命名
4. 查看数据摘要(是否需要清洗)
5. 查看数据频率,进行变量总结(特征向量+目标向量)
6. 是否缺失值
7. 声明特征向量,目标向量,进行数据集拆分(需要确定随机种子)
8. 进行训练,预测,进行多参数进行预测,查看最优解
9. 选择重要特征,删去无用特征,然后再次进行训练预测,寻找最佳策略
10. 使用`Confusion matrix`查看是分类者是犯了哪些错误,进行数据分析

## 9.总结

整体是构建了一个随机森林预测汽车安全,构建了两个模型,一个10个决策树,一个100决策树,后者对正确率是有提升(可能数据不太多,不是特别明显),因此,随着模型中决策树数量的藤甲,期望准确率也是随之增加了

在进行训练过程中,发现`doors`并没有任何影响,将其删除,再次进行训练,预测,正确率又有上升,可使用`Confusion matrix`进行数据分析,比较有道理
